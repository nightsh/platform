	...
	SetPostRepository
{
	...
	public function getEntity(Array $data = null)
	{
		...
		if ($data['form_id'])
		{
			...
			if ($this->canUserReadPostsValues($user, new Post($data), $this->form_repo)) {
		...
		if (!empty($data['id']))
		{
			...
			// @todo move or double up in formatter. That should enforce what users can see
			if ($this->canUserSeePostLock($user, new Post($data))) {
		...
		if ($data['author_realname'] || $data['user_id'] || $data['author_email'])
		{
			...
			if (!$this->canUserSeeAuthor($user, new Post($data), $this->form_repo))
			{
